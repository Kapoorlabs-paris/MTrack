...
public class LocalPrefs {
	...
	public static final String PREFS_NAME = "MTrack_Prefs.txt";
	...
	static Properties MTrackPrefs = new Properties();
	static Properties props = new Properties(MTrackPrefs);
	...
	*/
	public static String load(Object ij, Applet applet) {
		InputStream f = ij.getClass().getResourceAsStream("/"+PREFS_NAME);
		if (applet!=null)
			return loadAppletProps(f, applet);
		if (homeDir==null)
			homeDir = System.getProperty("user.dir");
		if (f==null) {
			try {f = new FileInputStream(homeDir+"/"+PREFS_NAME);}
			catch (FileNotFoundException e) {f=null;}
		}
		if (f==null)
			return PREFS_NAME+" not found in ij.jar or in "+homeDir;
		f = new BufferedInputStream(f);
		try {props.load(f); f.close();}
		catch (IOException e) {return("Error loading "+PREFS_NAME);}
	...
	static void dumpPrefs(String title) {
		IJ.log("");
		IJ.log(title);
		Enumeration e = MTrackPrefs.keys();
		while (e.hasMoreElements()) {
			String key = (String) e.nextElement();
			IJ.log(key+": "+MTrackPrefs.getProperty(key));
		}
	}
	*/
	...
	static String loadAppletProps(InputStream f, Applet applet) {
		if (f==null)
			return PREFS_NAME+" not found in ij.jar";
		try {
			props.load(f);
			f.close();
		}
	...
	public static void set(String key, String text) {
		...
		if (text==null)
			MTrackPrefs.remove(KEY_PREFIX+key);
		else
			MTrackPrefs.put(KEY_PREFIX+key, text);
	}
	...
	public static String get(String key, String defaultValue) {
		String value = MTrackPrefs.getProperty(KEY_PREFIX+key);
		if (value == null)
			return defaultValue;
		else
			return value;
	...
	public static double get(String key, double defaultValue) {
		String s = MTrackPrefs.getProperty(KEY_PREFIX+key);
		Double d = null;
		if (s!=null) {
			try {d = new Double(s);}
			catch (NumberFormatException e) {d = null;}
	...
	static void savePluginPrefs(Properties prefs) {
		...
		while (e.hasMoreElements()) {
			...
			if (key.indexOf(KEY_PREFIX) == 0)
				prefs.put(key, MTrackPrefs.getProperty(key));
		}
	}
