...
public class SingleThresholdHoughHFListener implements AdjustmentListener {
	final Label label;
	final float min, max;
	...
	public SingleThresholdHoughHFListener(final Interactive_MTSingleChannel parent , final Label label, final float min, final float max, final int scrollbarSize,
			final Scrollbar thresholdScrollbar) {
		this.label = label;
		this.min = min;
		this.parent = parent;
