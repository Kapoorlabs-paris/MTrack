...
public class SingleMaxSizeListener implements AdjustmentListener {
	final Label label;
	final float min, max;
	...
	public SingleMaxSizeListener(final Interactive_MTSingleChannel parent,final Label label, final float min, final float max, final int scrollbarSize,
			final Scrollbar maxsizeScrollbar) {
		this.label = label;
		this.min = min;
		this.max = max;
