...
public class TrackBatch {
	...
	public void Trackobject(final int next, final int endtime) {
		...
		for (int index = next; index <= endtime; ++index) {
			...
			if (parent.parent.FindLinesViaMSER) {
				...
				parent.returnVector = FindlinesVia.LinefindingMethodHF(groundframe, groundframepre,
						...
						parent.seedmap, parent.jpb, parent.thirdDimensionSize, parent.parent.maxdist, next,
						parent.numgaussians);
			...
			for (int seedID = MinSeedLabel; seedID <= MaxSeedLabel; ++seedID) {
				...
				for (int j = 0; j < plusminusstartlist.size(); ++j) {
					...
					if (plusminusstartlist.get(j).seedid == seedID) {
						...
						try {
							File fichier = new File(parent.batchfolder + "//" + parent.parent.addToName + "SeedLabel"
									+ seedID + plusminusstartlist.get(j).plusorminus + ".txt");
		...
		for (int seedID = MinSeedLabel; seedID <= MaxSeedLabel; ++seedID) {
			...
			for (int j = 0; j < plusminusstartlist.size(); ++j) {
				...
				if (plusminusendlist.get(j).seedid == seedID) {
					...
					try {
						File fichier = new File(parent.batchfolder + "//" + parent.parent.addToName + "SeedLabel"
								+ seedID + plusminusendlist.get(j).plusorminus + ".txt");
		...
		if (parent.analyzekymo) {
			...
			FileWriter deltaw;
			File fichierKydel = new File(parent.batchfolder + "//" + "Batch_Processed" + parent.parent.addToName
					+ "MTtracker-deltad" + ".txt");
